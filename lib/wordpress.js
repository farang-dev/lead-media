...
async function fetchPosts(page = 1, perPage = 20, categoryId) {
    if (!WP_API_URL) {
...
async function fetchRelatedPosts(currentPostId, count = 3, lang = 'en') {
    if (!WP_API_URL) {
        console.error('WordPress API URL not configured');
        return [];
    }
    try {
        ...
        if (!categoryIds || categoryIds.length === 0) {
            ...
            const response = await fetch(`${WP_API_URL}/posts?_embed=author&per_page=${count}&exclude=${currentPostId}&orderby=date&order=desc`, {
                next: { revalidate: 60 }, // Cache for a minute
            });
            if (!response.ok) {
        ...
        const response = await fetch(`${WP_API_URL}/posts?_embed=author&categories=${categoryIds.join(',')}&per_page=${count}&exclude=${currentPostId}`, {
            next: { revalidate: 60 }, // Cache for a minute
        });
        if (!response.ok) {
...
async function publishPost(title, content, status = 'publish', metaTitle, metaDescription, language = 'en' // Added language parameter
) {
    ...
    headers.append('Authorization', `Basic ${buffer_1.Buffer.from(`${WP_USERNAME}:${WP_PASSWORD}`).toString('base64')}`);
    const postData = {
        title,
